...
class Factory:
    ...
    def contributor(
        ...
        *,
        ...
        `relationship` defaults to 'other', which is what a contributor who has
        not said is — and, until the contribute form started asking, what every
        contributor in every memoir was. Pass a real group to test the question
        library, which picks what somebody is asked from it.
        """
        return self._one(
            """
            INSERT INTO memoir_participant
            ...
            RETURNING id, display_name, relationship::text AS relationship,
                      contributor_token, merged_into
            """,
            {
                "memoir": memoir_id,
                "name": display_name,
